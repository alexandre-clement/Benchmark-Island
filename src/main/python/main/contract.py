...
class Contract:
    ...
    def __init__(self, contract_amount=0, contract_resource=Resource.wood):
...
def get_contracts_dict(contracts):
    return {contract.resource: contract.amount for contract in contracts}
    ...
    for answer in range(1, len(json_object), 2):
        ...
        if action.get(JsonArguments.action) == JsonArguments.exploit:
            ...
            if exploited in contracts:
                if contracts[exploited] > 0 >= contracts[exploited] - amount:
                    completed[exploited] = int(answer / 2)
                contracts[exploited] -= amount
    return completed
